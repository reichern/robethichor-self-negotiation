...
PING_LOG_FILE=/home/gssi-lab/robethichor_ws/results/ping_info.txt
 
for N in "${N_VALUES[@]}"; do
  for P in "${P_VALUES[@]}"; do
    echo "***********RUNNING $N - $P***********"
    read -p "Update the wait time for the configuration $N - $P (default = $WAIT_TIME): " WAIT_TIME
 
    echo -e "\n\n\n\n\n\n******PING BEFORE RUNNING $N - $P*****" >> $PING_LOG_FILE
 
    echo "Checking current PING to $R1_HOST"
    ping -c 10 $R1_HOST >> $PING_LOG_FILE
 
    echo "Checking current PING to $R2_HOST"
    ping -c 10 $R2_HOST >> $PING_LOG_FILE
    counter=$STARTGING_COUNTER
